...
vector<string> PingyControlPanel::theSerials = vector<string>();

vector<string>& PingyControlPanel::updateSerialList(){
	theSerials.clear();
...
PingyControlPanel::GazeValues PingyControlPanel::syncGazeValues = GazeValues();
PingyControlPanel::DanceValues PingyControlPanel::syncDanceValues = DanceValues();

set<PingyControlPanel*> PingyControlPanel::theSyncGazePanels = set<PingyControlPanel*>();
set<PingyControlPanel*> PingyControlPanel::theSyncDancePanels = set<PingyControlPanel*>();

PingyControlPanel::PingyControlPanel(const ofVec2f p):
mGui(p.x,p.y,0,0) {
	bDelete = false;
	bSerialInited = false;
	...
	ofAddListener(mGui.newGUIEvent,this,&PingyControlPanel::guiListener);
}

PingyControlPanel::~PingyControlPanel(){
	mSerial.close();
	theSyncGazePanels.erase(this);
	theSyncDancePanels.erase(this);
}

void PingyControlPanel::update(){
...
void PingyControlPanel::guiListener(ofxUIEventArgs &args){
...
const bool& PingyControlPanel::toDelete() const{
	return bDelete;
}

void PingyControlPanel::setX(const int x_){
	mGui.setPosition(x_, mGui.getRect()->y);
}

const ofRectangle PingyControlPanel::getRectangle() {
	return ofRectangle(mGui.getRect()->x,mGui.getRect()->y,mGui.getRect()->width,mGui.getRect()->height);
}

//// helpers
void PingyControlPanel::sendPanAndTilt() {
	if(bSerialInited){
...
void PingyControlPanel::sendSide() {
	if(bSerialInited){
		string msg;
...
//// static helpers
void PingyControlPanel::sendSyncPanAndTilt() {
	for(set<PingyControlPanel*>::const_iterator it=theSyncGazePanels.begin(); it!=theSyncGazePanels.end(); ++it){
		(*it)->mGazeValues = syncGazeValues;
		(*it)->sendPanAndTilt();
		(*it)->bUpdateGazeGuiFromValues = true;
	}
}
void PingyControlPanel::sendSyncPanSpeed() {
	for(set<PingyControlPanel*>::const_iterator it=theSyncGazePanels.begin(); it!=theSyncGazePanels.end(); ++it){
		(*it)->mGazeValues = syncGazeValues;
		(*it)->sendPanSpeed();
		(*it)->bUpdateGazeGuiFromValues = true;
...
void PingyControlPanel::sendSyncTiltSpeed() {
	for(set<PingyControlPanel*>::const_iterator it=theSyncGazePanels.begin(); it!=theSyncGazePanels.end(); ++it){
		(*it)->mGazeValues = syncGazeValues;
		(*it)->sendTiltSpeed();
		(*it)->bUpdateGazeGuiFromValues = true;
...
void PingyControlPanel::sendSyncSideSpeed() {
	for(set<PingyControlPanel*>::const_iterator it=theSyncGazePanels.begin(); it!=theSyncGazePanels.end(); ++it){
		(*it)->mGazeValues = syncGazeValues;
		(*it)->sendSideSpeed();
		(*it)->bUpdateGazeGuiFromValues = true;
...
void PingyControlPanel::syncDance() {
	for(set<PingyControlPanel*>::const_iterator it=theSyncDancePanels.begin(); it!=theSyncDancePanels.end(); ++it){
