...
void testApp::setup(){
	...
	mainGui.addWidgetDown(new ofxUILabel("Pingy Controller", OFX_UI_FONT_MEDIUM));
	mainGui.addSpacer(mainGui.getRect()->width,4);
	mainGui.addWidgetDown(new ofxUILabelButton("Add a Pingy", false));
...
void testApp::update(){
	...
	for(int i=thePanels.size()-1; i>-1; --i){
		PingyControlPanel* mp = thePanels.at(i);
		if(mp->toDelete()){
			delete mp;
...
void testApp::mainGuiEvent(ofxUIEventArgs &e){
	...
	if((name.compare("Add a Keepon") == 0) && (((ofxUIButton*)e.widget)->getValue())){
		...
		PingyControlPanel* mp = new PingyControlPanel(ofVec2f(0,mainGui.getRect()->y+mainGui.getRect()->height+10));
